...
# 4. TEXT-TO-VIDEO SCHEMA (Wan2.1 T2V) - COMPLETELY SEPARATE!
TEXT2VIDEO_SCHEMA = {
    'prompt': {
        'type': str,
        'required': True,
...
def get_schema_for_task_type(task_type):
    """
    Get the appropriate schema based on task type
    """
    schema_map = {
    ...
    if task_type in schema_map:
        return schema_map[task_type]
    else:
        # Default to text2img for backward compatibility
        return TEXT2IMG_SCHEMA

# ============================================================================
# LEGACY UNIFIED SCHEMA (DEPRECATED - FOR BACKWARD COMPATIBILITY ONLY)
# This should be REMOVED once the handler is updated to use separate schemas
# ============================================================================
...
INPUT_SCHEMA = {
    ...
    # VIDEO PARAMETERS - SHOULD NOT BE HERE! (Only for legacy compatibility)
    'video_height': {
        'type': int,
        'required': False,
        'default': None,  # Don't default for image requests
        'constraints': lambda x: x is None or x in [480, 720]
    },
    'video_width': {
        'type': int,
        'required': False,
        'default': None,  # Don't default for image requests
        'constraints': lambda x: x is None or x in [832, 1280]
    },
    'num_frames': {
        'type': int,
        'required': False,
    ...
    'video_guidance_scale': {
        ...
        'required': False,
        'default': None,  # Don't default for image requests
        'constraints': lambda x: x is None or (1.0 <= x <= 20.0)
    },
    'fps': {
        'type': int,
        'required': False,
        'default': None,  # Don't default for image requests
        'constraints': lambda x: x is None or (6 <= x <= 30)
    },
}
